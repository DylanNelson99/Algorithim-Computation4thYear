...
string TextCompression::encode_input_text(const string& input, ofstream& out_file, HuffmanTree*& huffman_tree_out) {
	string encode_string = encodeText(input, huffman_tree_out);
	out_file << encode_string;
...
HuffmanTree* TextCompression::encode_input_to_file(ifstream& in_file, ofstream& out_file) {
	...
	string encode_string = encode_input_text(input_text, out_file, huffman_tree);
...
HuffmanTree* TextCompression::compress_input(ifstream& in_file, ofstream& out_file) {
	...
	string encode_string = encodeText(input_text, huffman_tree_out);

	//Compress String
	string compress_string_out;
	get_compressed_string(encode_string, compress_string_out);
...
string TextCompression::encodeText(const string & input, HuffmanTree * &huffman_tree_out) {
	...
	string encode_string = encode_char_map(input, char_encoding);
	return encode_string;
}
...
string TextCompression::encode_char_map(const string & input, const map<int, string>&char_encoding) {
	...
	for (int input_char : input) {
		encode_string.append(char_encoding.at(input_char));
	}
	//.append Pseudo_eof at the end of string
	encode_string.append(char_encoding.at(PSEUDO_EOF));		
	return encode_string;
}

map<int, string> TextCompression::get_mapping_from_tree(HuffmanTree * huffman_tree, map<int, int> char_freq) {
...
HuffmanTree* TextCompression::get_huffman_tree_from_map(map<int, int> char_freq)
{
	auto tree_queue = get_priority_queue_from_map(char_freq);						
	...
	while (tree_queue.size() > 1) {									
		...
		tree_queue.push(new HuffmanTree(first_pop, second_pop, first_pop->size + second_pop->size));
	}
...
TextCompression::get_priority_queue_from_map(map<int, int> char_freq) {
...
void TextCompression::populate_char_frequency(const string & input, map<int, int>&char_freq) {
...
*/
void TextCompression::get_compressed_string(string & encode_string, string & compress_string_out) {
	pad_string_with_zeros(encode_string);

	compress_string_out = "";
	istringstream encoded_stream(encode_string);
	string output;
...
*/
void TextCompression::get_binary_string(const string & encode_string, string & binary_string_out) {
	binary_string_out = "";
	for (char encoded_char : encode_string) {
...
*/
string TextCompression::encode_huffmantree(HuffmanTree * huffman_tree) {
	string encode_string = "";
	encode_huffmantree_helper(huffman_tree, encode_string);
	encode_string.append("0");
	return encode_string;
}

string TextCompression::encode_huffmantree_helper(HuffmanTree * huffman_tree, string & acc_string) {
...
HuffmanTree* TextCompression::decode_huffman_tree(const string & encoded_tree, const unsigned int binary_bits) {
